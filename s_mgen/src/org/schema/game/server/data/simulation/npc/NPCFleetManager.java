...
public class NPCFleetManager implements LogInterface{
    ...
    public Fleet spawnTradingFleet(ElementCountMap c, Vector3i from, Vector3i to) {

        String fleetName = getName(stats.getFleetCreatorNumber(), FleetType.TRADING);
        ...
        for(Fleet f : fleets){
            if(f.getName().equals(fleetName)){
                ...
                Vector3i pos = findUnloadedOnWay(from, to);
    ...
    //inserted code
    public Fleet spawnFleet(Vector3i target, int size) {
        FleetType fleetType = FleetType.ATTACKING;

        String fleetName = getName(stats.getFleetCreatorNumber(), fleetType);

        getState().getFleetManager().requestCreateFleet(fleetName, getOwnerName());
        stats.incFleetNumber();
        recalcFleets();

        for(Fleet thisFleet : fleets){
            if(thisFleet.getName().equals(fleetName)){

                Vector3i pos = findUnloadedOnWay(faction.getHomeSector(), target);
                boolean spawned = fillFleets(thisFleet, fleetType, size, pos);
                if(spawned){
                    FleetCommand com = new FleetCommand(FleetCommandTypes.PATROL_FLEET, thisFleet, target);
                    getFleetManager().executeCommand(com);
                    return thisFleet;
                }else{
                    removeFleetAndPurgeShips(thisFleet);
                    recalcFleets();
                }
                return null;
            }
    ...
    private boolean fillFleets(Fleet fleet, FleetType type, int size, Vector3i posToSpawn) {
        ...
        if(bbs.size() > 0){
            ...
            for(int i = 0; i < size; i++){
                ...
                String UID = "GFLTSHP_"+getFaction().getIdFaction()+"_"+stats.getShipCreatorNumber();
                stats.incShipNumber();
    ...
    public void fleetTurn() {
        ...
        for(int i = 0; i < getFaction().getConfig().scvengingFleetsPerTurn; i++){
            Vector3i closest = null;
            float dist = 0;
            for(Vector3i sector : attackSector){
                float d = Vector3i.getDisatance(getFaction().getHomeSector(), sector);
            ...
            if(closest != null){
                ...
                float range = Math.max(getFaction().getConfig().minScavenginRange, (getFaction().structure.getLastHabitatedLevel() * getFaction().getConfig().scavengingRangePerFactionLevel));

                if(Vector3i.getDisatance(getFaction().getHomeSector(), closest)/16.0f < range){
